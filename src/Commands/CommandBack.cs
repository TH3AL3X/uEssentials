...
namespace Essentials.Commands
{
    ...
    public class CommandBack : EssCommand
    {
        ...
        public override CommandResult OnExecute(ICommandSource src, ICommandArgs args)
        {
            ...
            {
                return CommandResult.LangError("icon_error_general", "NOT_DIED_YET");
            }
            ...
            {
                return CommandResult.LangError("icon_error_general", "BACK_DELAY", TimeUtil.FormatSeconds((uint)delta));
            }

            var backPosition = playerMeta.Get<Vector3>(META_KEY_POS);
            src.ToPlayer().Teleport(backPosition);
            EssLang.Send("icon_set_returned", src, "RETURNED");
