...
namespace Essentials.Commands
{
    ...
    public class CommandReputation : EssCommand
    {
        ...
        public override CommandResult OnExecute(ICommandSource src, ICommandArgs args)
        {
            ...
            if (args.Length == 2)
            {
                if (args[1].RawValue == "*")
                {
                    ...
                    if (amount >= 0)
                    {
                        EssLang.Send("generalicon", src, "REPUTATION_GIVEN", amount, EssLang.Translate("EVERYONE"));
                    }
                    else
                    {
                        EssLang.Send("generalicon", src, "REPUTATION_TAKE", -amount, EssLang.Translate("EVERYONE"));
                    }
                }
                ...
                else
                {
                    ...
                    if (amount >= 0)
                    {
                        EssLang.Send("generalicon", src, "REPUTATION_GIVEN", amount, player.DisplayName);
                    }
                    else
                    {
                        EssLang.Send("generalicon", src, "REPUTATION_TAKE", -amount, player.DisplayName);
                    }
        ...
        private static void GiveRep(UPlayer player, int amount)
        {
            ...
            if (amount >= 0)
            {
                EssLang.Send("generalicon", player, "REPUTATION_RECEIVED", amount);
            }
            else
            {
                EssLang.Send("generalicon", player, "REPUTATION_LOST", -amount);
            }
        }
