...
namespace Essentials.Api.Unturned
{
    ...
    public sealed class UPlayer : ICommandSource
    {
        ...
        {
            ChatManager.serverSendMessage(message?.ToString() ?? "null", color, null, null, EChatMode.SAY, null, true);
        }

        public void SendMessage(object message)
        {
            var color2 = Color.red;
            ChatManager.serverSendMessage(message?.ToString() ?? "null", color2, null, null, EChatMode.SAY, null, true);
